...
('HSBC Large & Mid Cap Fund', 'An open-ended equity scheme investing in both large-cap and mid-cap stocks.',
(SELECT id FROM MUTUAL_FUND_HOUSE WHERE name = 'HSBC'),
(SELECT id FROM FUND_TYPE WHERE name = 'LARGE_MID_CAP')),

('HSBC Multi Cap Fund', 'A multi-cap fund investing across large-cap, mid-cap, and small-cap stocks.',
(SELECT id FROM MUTUAL_FUND_HOUSE WHERE name = 'HSBC'),
(SELECT id FROM FUND_TYPE WHERE name = 'MULTI_CAP')),

('HSBC Nifty 50 Index Fund', 'An open-ended equity scheme replicating/tracking the Nifty 50 Index.',
(SELECT id FROM MUTUAL_FUND_HOUSE WHERE name = 'HSBC'),
(SELECT id FROM FUND_TYPE WHERE name = 'INDEX')),

('HSBC Consumption Fund', 'An open-ended equity scheme following the consumption theme.',
(SELECT id FROM MUTUAL_FUND_HOUSE WHERE name = 'HSBC'),
...
('HSBC ELSS Tax Saver Fund', 'An open-ended equity-linked saving scheme with a statutory lock-in of 3 years and tax benefit.',
(SELECT id FROM MUTUAL_FUND_HOUSE WHERE name = 'HSBC'),
